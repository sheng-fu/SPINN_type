...
class YFOptimizer(object):
    ...
    def curvature_range(self):
        global_state = self._global_state
        if self._iter == 0:
            global_state["curv_win"] = torch.FloatTensor(
                self._curv_win_width, 1).zero_()
        ...
        if self._iter == 0:
            ...
            self._h_min = 0.0
            self._h_max = 0.0
        global_state["h_min_avg"] = global_state["h_min_avg"] * \
            beta + (1 - beta) * torch.min(curv_win[:valid_end])
        global_state["h_max_avg"] = global_state["h_max_avg"] * \
            beta + (1 - beta) * torch.max(curv_win[:valid_end])
    ...
    def grad_variance(self):
        ...
        for group in self._optimizer.param_groups:
            for p in group['params']:
                ...
                self._grad_var += torch.sum(state["grad_avg"]
                                            * state["grad_avg"])
    ...
    def dist_to_opt(self):
        ...
        global_state["grad_norm_avg"] = global_state["grad_norm_avg"] * \
            beta + (1 - beta) * math.sqrt(global_state["grad_norm_squared"])
        global_state["dist_to_opt_avg"] = global_state["dist_to_opt_avg"] * beta + \
    ...
    def after_apply(self):
        ...
        global_state['grad_norm_squared_avg'] = global_state['grad_norm_squared_avg'] * \
            beta + (1 - beta) * global_state['grad_norm_squared']
    ...
    def get_mu(self):
        ...
        coef[2] = -(3 + self._dist_to_opt**2 *
                    self._h_min**2 / 2 / self._grad_var)
        roots = np.roots(coef)
        root = roots[np.logical_and(np.logical_and(
            np.real(roots) > 0.0, np.real(roots) < 1.0), np.imag(roots) < 1e-5)]
        assert root.size == 1
        dr = self._h_max / self._h_min
        self._mu_t = max(np.real(root)[0]**2,
                         ((np.sqrt(dr) - 1) / (np.sqrt(dr) + 1))**2)
        return
